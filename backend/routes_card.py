...
# get student linked to a card uid - used by the pi scan script
@cards_bp.get("/cards/<string:card_uid>/student")
def get_student_by_card(card_uid):
    card = Card.query.filter_by(card_uid=card_uid).first()
    if not card:
        return jsonify({"error": "not_found", "message": "Card not found"}), 404
